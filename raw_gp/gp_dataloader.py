...
class DataLoader:    
    def normalizeCols(self,data, cols):
        ...
        all_values  =  temp_df.loc[:,columns].values.flatten()

        mean = all_values.mean()
        std = all_values.std()

        temp_df.loc[:,columns] = (temp_df.loc[:, columns] - mean)/(std + 1e-9)
    ...
    def getModelInputsAndOutputs(self):
        ...
        self.snotel_locations_info['mapped_koppen_code'] = self.snotel_locations_info['Koppen_code_ID'].map({val: index for index, val in enumerate(np.unique(self.snotel_locations_info['Koppen_code_ID']))})
        self.snotel_locations_info['mapped_landcover_code'] = self.snotel_locations_info['Lancover_code'].map({val: index for index, val in enumerate(np.unique(self.snotel_locations_info['Lancover_code']))})
        ...
        for index,location_info in self.snotel_locations_info.iterrows():
            ...
            spatial_feature = np.array([lat,longitude,elev,southness, location_info['mapped_koppen_code'] , location_info['mapped_landcover_code']])
    ...
    def getAllInputAndTargets(self):
        self.cleanDataJunk()
        self.normalizeData()
        self.addDayInfo()

        self.model_inputs, self.model_outputs = self.getModelInputsAndOutputs()
        self.completeInputsAndOutputs()

        return self.model_inputs,self.model_outputs, self.all_dates
