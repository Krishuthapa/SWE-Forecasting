...
def getTestsResults(test_loader):
    ...
    for index in range(0,len(means), FORECASTING_WINDOW):
...
def run_and_save_result(inputs, outputs, year):
    ...
    joblib.dump(window_means, '{}/results/predict/{}_daily_raw_gp_w_predict.pkl'.format(baseUrl, year))
    joblib.dump(window_low, '{}/results/predict/{}_daily_raw_gp_w_low_predict.pkl'.format(baseUrl, year))
    joblib.dump(window_high, '{}/results/predict/{}_daily_raw_gp_w_high_predict.pkl'.format(baseUrl, year))
    joblib.dump(means, '{}/results/predict/{}_daily_raw_gp_predict.pkl'.format(baseUrl,year))
    joblib.dump(low_values, '{}/results/predict/{}_daily_raw_gp_lower_predict.pkl'.format(baseUrl, year))
    joblib.dump(high_values, '{}/results/predict/{}_daily_raw_gp_higher_predict.pkl'.format(baseUrl,year))
