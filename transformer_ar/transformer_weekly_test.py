...
hasFile = os.path.isfile("{}/checkpoints/forecasting_weekly_tf_model.pt".format(BASE_URL))

if hasFile:
    checkpoint = torch.load("{}/checkpoints/forecasting_weekly_tf_model.pt".format(BASE_URL))
...
def TestModelSP(testing_yr_indices, all_inputs, all_outputs, all_dates, batch_size = 128):
    ...
    with torch.no_grad():
        for index , input_index in enumerate(testing_yr_indices): 
            ...
            instance_daily_input  = getInputs(all_inputs, input_index, all_indices, short_window = 365, step_size = 1)
            ...
            decoder_past_x = getPastOutputs(all_outputs, input_index, all_indices, window = 8, step_size = FORECASTING_STEP_SIZE)
...
def getDailyPredictions(data_indices, all_inputs,all_outputs,all_dates, batch_size = 128):
    ...
    with torch.no_grad():
        ...
        for data_index in range(0,len(data_indices), FORECASTING_WINDOW * FORECASTING_STEP_SIZE):
            ...
            decoder_past_x = getPastOutputs(all_outputs, data_indices[data_index], all_indices, window = 8, step_size = FORECASTING_STEP_SIZE)
            decoder_past_x = decoder_past_x.to(device=device, dtype=torch.float32)
            decoder_past_x = ((decoder_past_x - min_swe)/(max_swe - min_swe))
...
print("Test Set 1")
weekly_window_actual_1, weekly_window_predicted_1  = TestModelSP(testing_yr_indices_1, model_inputs, model_outputs, all_dates)
weekly_actual_1, weekly_predicted_1  = getDailyPredictions(testing_yr_indices_1, model_inputs, model_outputs, all_dates)

print("================================================")

print("Test Set 2")
weekly_window_actual_2, weekly_window_predicted_2 = TestModelSP(testing_yr_indices_2, model_inputs, model_outputs, all_dates)
weekly_actual_2, weekly_predicted_2  = getDailyPredictions(testing_yr_indices_2, model_inputs, model_outputs, all_dates)

print("================================================")

print("Test Set 3")
weekly_window_actual_3, weekly_window_predicted_3 = TestModelSP(testing_yr_indices_3, model_inputs, model_outputs, all_dates)
weekly_actual_3, weekly_predicted_3  = getDailyPredictions(testing_yr_indices_3, model_inputs, model_outputs, all_dates)

print("================================================")

print("Test Set 4")
weekly_window_actual_4, weekly_window_predicted_4 = TestModelSP(testing_yr_indices_4, model_inputs, model_outputs, all_dates)
weekly_actual_4, weekly_predicted_4  = getDailyPredictions(testing_yr_indices_4, model_inputs, model_outputs, all_dates)

print("================================================")

print("Test Set 5")
weekly_window_actual_5, weekly_window_predicted_5 = TestModelSP(testing_yr_indices_5, model_inputs, model_outputs, all_dates)
weekly_actual_5, weekly_predicted_5  = getDailyPredictions(testing_yr_indices_5, model_inputs, model_outputs, all_dates)

print("================================================")

joblib.dump(weekly_window_predicted_1, '{}/results/predict/2014_weekly_w_transformer_predict.pkl'.format(BASE_URL))
joblib.dump(weekly_window_predicted_2, '{}/results/predict/2015_weekly_w_transformer_predict.pkl'.format(BASE_URL))
joblib.dump(weekly_window_predicted_3, '{}/results/predict/2016_weekly_w_transformer_predict.pkl'.format(BASE_URL))
joblib.dump(weekly_window_predicted_4, '{}/results/predict/2017_weekly_w_transformer_predict.pkl'.format(BASE_URL))
joblib.dump(weekly_window_predicted_5, '{}/results/predict/2018_weekly_w_transformer_predict.pkl'.format(BASE_URL))

joblib.dump(weekly_predicted_1, '{}/results/predict/2014_weekly_transformer_predict.pkl'.format(BASE_URL))
joblib.dump(weekly_predicted_2, '{}/results/predict/2015_weekly_transformer_predict.pkl'.format(BASE_URL))
joblib.dump(weekly_predicted_3, '{}/results/predict/2016_weekly_transformer_predict.pkl'.format(BASE_URL))
joblib.dump(weekly_predicted_4, '{}/results/predict/2017_weekly_transformer_predict.pkl'.format(BASE_URL))
joblib.dump(weekly_predicted_5, '{}/results/predict/2018_weekly_transformer_predict.pkl'.format(BASE_URL))

joblib.dump(weekly_window_actual_1, '{}/results/actual/2014_weekly_w_transformer_actual.pkl'.format(BASE_URL))
joblib.dump(weekly_window_actual_2, '{}/results/actual/2015_weekly_w_transformer_actual.pkl'.format(BASE_URL))
joblib.dump(weekly_window_actual_3, '{}/results/actual/2016_weekly_w_transformer_actual.pkl'.format(BASE_URL))
joblib.dump(weekly_window_actual_4, '{}/results/actual/2017_weekly_w_transformer_actual.pkl'.format(BASE_URL))
joblib.dump(weekly_window_actual_5, '{}/results/actual/2018_weekly_w_transformer_actual.pkl'.format(BASE_URL))

joblib.dump(weekly_actual_1, '{}/results/actual/2014_weekly_transformer_actual.pkl'.format(BASE_URL))
joblib.dump(weekly_actual_2, '{}/results/actual/2015_weekly_transformer_actual.pkl'.format(BASE_URL))
joblib.dump(weekly_actual_3, '{}/results/actual/2016_weekly_transformer_actual.pkl'.format(BASE_URL))
joblib.dump(weekly_actual_4, '{}/results/actual/2017_weekly_transformer_actual.pkl'.format(BASE_URL))
joblib.dump(weekly_actual_5, '{}/results/actual/2018_weekly_transformer_actual.pkl'.format(BASE_URL))
