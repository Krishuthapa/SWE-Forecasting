...
# Optimizer
optimizer_sp = torch.optim.AdamW(parameters, lr= 0.005, weight_decay = 0.01)
scheduler_sp = torch.optim.lr_scheduler.StepLR(optimizer_sp, step_size = 3 ,gamma = 0.45, last_epoch= -1, verbose=False)
...
def TrainModelSP(all_inputs, all_outputs, training_yr_indices, epoch_number, batch_size = 32):
    ...
    for index, input_index in enumerate(training_yr_indices):
        ...
        instance_daily_input  = getInputs(all_inputs, input_index, all_indices, short_window= 30, step_size = 1)
        ...
        decoder_past_x = getPastOutputs(all_outputs, input_index, all_indices, window = 6, step_size = FORECASTING_STEP_SIZE)
...
while strike_count <= 3 and epoch_number <= 8:
    ...
    if epoch_loss_sp < best_nse:
        ...
        torch.save({
                ...
                }, "{}/checkpoints/forecasting_weekly_tf_model.pt".format(BASE_URL))
