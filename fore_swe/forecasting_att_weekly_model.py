...
class CrossVarTransformer(nn.Module):
    def __init__(self, encoder_input_dim = 13 , batch_size = 7, sp_tp_dim = 6,  model_dim = 512, prompt_dim = 1536, enc_layers_count = 8, nhead = 16):
        ...
        self.variable_attn = nn.MultiheadAttention(self.model_dim, self.nhead, batch_first = True)

        self.encoder_layers = nn.ModuleList([CustomEncoderLayer(d_model = self.model_dim, num_heads= self.nhead) for _ in range(self.enc_layers_count)])
    ...
    def forward(self,encoder_inputs, prompt_inputs, distance_pairs, angularity_pairs):
        ...
        var_encoded_input_x, _ = self.variable_attn(query_embedding, embedded_var_input_x, embedded_var_input_x)
        var_encoded_input_x = var_encoded_input_x.squeeze(1).reshape(1, sequence_length, self.model_dim)
    
        # ===================================================================================

        final_output = var_encoded_input_x + query_embedding.permute(1,0,2)
        
        return final_output.reshape(1,sequence_length, self.model_dim)
...
# Optimizer
optimizer_sp = torch.optim.AdamW(parameters, lr= 0.000005, weight_decay = 0.0001)
scheduler_sp = torch.optim.lr_scheduler.StepLR(optimizer_sp, step_size = 4 ,gamma = 0.65, last_epoch= -1, verbose=False)
...
hasFile = os.path.isfile("{}/checkpoints/forecasting_modified_weekly_att_climax_model_gp_act.pt".format(baseUrl))
...
with open(output_1, 'w') as file1:
    with redirect_stdout(file1):
        ...
        if hasFile:
            checkpoint = torch.load("{}/checkpoints/forecasting_modified_weekly_att_climax_model_gp_act.pt".format(baseUrl))
        ...
        while strike_count <= 3 and epoch_number <= 6:
            ...
            if epoch_loss_sp < best_nse:
                ...
                }, "{}/checkpoints/forecasting_modified_weekly_att_climax_model_gp_act.pt".format(baseUrl))
...
def getFinalInputsForGP(training_yr_indices, train_prompts):
    ...
    torch.save(input_learnt_representations, '{}/outputs/forecasting_modified_weekly_att_inputs_for_gp_act.pt'.format(baseUrl))
    torch.save(predicted_outputs, '{}/outputs/forecasting_modified_weekly_att_predicted_outputs_for_gp_act.pt'.format(baseUrl))
    torch.save(actual_outputs, '{}/outputs/forecasting_modified_weekly_att_outputs_for_gp_act.pt'.format(baseUrl))
...
daily_forecasted_2014 = joblib.load('{}/results/predict/2014_daily_predict.pkl'.format(baseUrl))
daily_forecasted_2015 = joblib.load('{}/results/predict/2015_daily_predict.pkl'.format(baseUrl))
daily_forecasted_2016 = joblib.load('{}/results/predict/2016_daily_predict.pkl'.format(baseUrl))
daily_forecasted_2017 = joblib.load('{}/results/predict/2017_daily_predict.pkl'.format(baseUrl))
daily_forecasted_2018 = joblib.load('{}/results/predict/2018_daily_predict.pkl'.format(baseUrl))
...
torch.save(test_1_final_representations, '{}/outputs/weekly_test1_inputs_for_gp_act.pt'.format(baseUrl))
torch.save(test_2_final_representations, '{}/outputs/weekly_test2_inputs_for_gp_act.pt'.format(baseUrl))
torch.save(test_3_final_representations, '{}/outputs/weekly_test3_inputs_for_gp_act.pt'.format(baseUrl))
torch.save(test_4_final_representations, '{}/outputs/weekly_test4_inputs_for_gp_act.pt'.format(baseUrl))
torch.save(test_5_final_representations, '{}/outputs/weekly_test5_inputs_for_gp_act.pt'.format(baseUrl))
