...
output_1 = '{}/outputs/forecasting_vt_att_tr_act_loss.txt'.format(baseUrl)
output_2 = '{}/outputs/forecasting_vt_att_ts_act_loss.txt'.format(baseUrl)
...
#test_indices = [value for value in range(512)]
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        """
        Arguments:
            x_input: Tensor, shape ``[batch_size, seq_len, embedding_dim]``
            x_expexted: Tensor, shape ``[seq_len, batch_size, embedding_dim]``
        """
...
class SweAttetnionTemporal(nn.Module):
    def __init__(self, input_dim, sp_dim, prompt_dim, model_dim, output_dim, num_layers, nhead):
        ...
        self.observation_embedding_1 = nn.Linear(input_dim, int(model_dim/2))
        self.observation_embedding_2 = nn.Linear(int(model_dim/2), model_dim)

        self.positonal_embedding = PositionalEncoding(d_model = model_dim)
        ...
        self.decoder = nn.Linear(2 * model_dim , output_dim)

    def forward(self, x_daily, x_yearly, prompts):
        ...
        daily_obs_embedding = self.observation_embedding_1(source_daily.reshape(batch_size * days_count,-1))
        ...
        daily_obs_embedding = self.observation_embedding_2(daily_obs_embedding)
        daily_obs_embedding = self.positonal_embedding(daily_obs_embedding.reshape(batch_size, days_count, self.model_dim))

        source_yearly = x_yearly[:,:,4:]
        yearly_obs_embedding = self.observation_embedding_1(source_yearly.reshape(batch_size * years_count, -1))
        ...
        yearly_obs_embedding = self.observation_embedding_2(yearly_obs_embedding)
        yearly_obs_embedding = self.positonal_embedding(yearly_obs_embedding.reshape(batch_size, years_count, self.model_dim))
        ...
        selected_daily_obs_encoding = daily_obs_encoding[:,-1,:].reshape(1, batch_size, self.model_dim)
        selected_daily_obs_encoding = selected_daily_obs_encoding + location_embedding

        selected_yearly_obs_encoding = yearly_obs_encoding[:,-1,:].reshape(1,batch_size, self.model_dim)
        selected_yearly_obs_encoding = selected_yearly_obs_encoding + location_embedding

        final_encoded_info = torch.cat((selected_daily_obs_encoding, selected_yearly_obs_encoding), dim = -1)
        
        outputs = self.decoder(final_encoded_info.reshape(-1,2* self.model_dim))

        return outputs.reshape(1,batch_size, self.output_dim)

swe_model = SweAttetnionTemporal(input_dim = 15, sp_dim = 4, prompt_dim = 1536, model_dim = 1024, nhead = 16, output_dim = 10, num_layers = 6)
...
def TrainModelSP(training_yr_indices, loc_prompts, location_distances, location_angularities, epoch_number):
    ...
    for index, input_index in enumerate(training_yr_indices):
        ...
        instance_daily_input  = getInputs(input_index, all_indices, window = 120, step_size = 1)
        ...
        output = swe_model(instance_daily_input, instance_yearly_input, loc_prompts)
...
hasFile = os.path.isfile("{}/checkpoints/forecasting_daily_vanilla_att_temp_model.pt".format(baseUrl))
...
with open(output_1, 'w') as file1:
    with redirect_stdout(file1):
        ...
        if hasFile:
            checkpoint = torch.load("{}/checkpoints/forecasting_daily_vanilla_att_temp_model.pt".format(baseUrl))
        ...
        while strike_count <= 3 and epoch_number <= 10:
            ...
            if epoch_loss_sp < best_nse:
                ...
                }, "{}/checkpoints/forecasting_daily_vanilla_att_temp_model.pt".format(baseUrl))
...
def TestModelSP(testing_yr_indices):
    ...
    with torch.no_grad():
        for index , input_index in enumerate(testing_yr_indices): 
            ...
            output = swe_model(instance_daily_input, instance_yearly_input, all_prompts)
...
def getDailyPredictions(data_indices):
    ...
    for data_index in range(0,len(data_indices), forecasting_window * forecasting_step_size):        
        instance_daily_input  = getInputs(data_indices[data_index], all_indices, window = 120, step_size = 1)
...
with open(output_2, 'w') as file1:
    with redirect_stdout(file1):
        print("Test Set 1")
        daily_window_actual_1, daily_window_predicted_1  = TestModelSP(testing_yr_indices_1)
        daily_actual_1, daily_predicted_1 = getDailyPredictions(testing_yr_indices_1)
        print("================================================")

        print("Test Set 2")
        daily_window_actual_2, daily_window_predicted_2 = TestModelSP(testing_yr_indices_2)
        daily_actual_2, daily_predicted_2 = getDailyPredictions(testing_yr_indices_2)
        print("================================================")
        
        print("Test Set 3")
        daily_window_actual_3, daily_window_predicted_3 = TestModelSP(testing_yr_indices_3)
        daily_actual_3, daily_predicted_3 = getDailyPredictions(testing_yr_indices_3)
        print("================================================")
        
        print("Test Set 4")
        daily_window_actual_4, daily_window_predicted_4 = TestModelSP(testing_yr_indices_4)
        daily_actual_4, daily_predicted_4 = getDailyPredictions(testing_yr_indices_4)
        print("================================================")
        
        print("Test Set 5")
        daily_window_actual_5, daily_window_predicted_5 = TestModelSP(testing_yr_indices_5)
...
joblib.dump(daily_window_predicted_1, '{}/results/predict/2014_daily_window_predict_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_window_predicted_2, '{}/results/predict/2015_daily_window_predict_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_window_predicted_3, '{}/results/predict/2016_daily_window_predict_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_window_predicted_4, '{}/results/predict/2017_daily_window_predict_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_window_predicted_5, '{}/results/predict/2018_daily_window_predict_vt_att.pkl'.format(baseUrl))

joblib.dump(daily_window_actual_1, '{}/results/actual/2014_daily_window_actual_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_window_actual_2, '{}/results/actual/2015_daily_window_actual_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_window_actual_3, '{}/results/actual/2016_daily_window_actual_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_window_actual_4, '{}/results/actual/2017_daily_window_actual_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_window_actual_5, '{}/results/actual/2018_daily_window_actual_vt_att.pkl'.format(baseUrl))

joblib.dump(daily_predicted_1, '{}/results/predict/2014_daily_predict_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_predicted_2, '{}/results/predict/2015_daily_predict_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_predicted_3, '{}/results/predict/2016_daily_predict_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_predicted_4, '{}/results/predict/2017_daily_predict_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_predicted_5, '{}/results/predict/2018_daily_predict_vt_att.pkl'.format(baseUrl))

joblib.dump(daily_actual_1, '{}/results/actual/2014_daily_actual_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_actual_2, '{}/results/actual/2015_daily_actual_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_actual_3, '{}/results/actual/2016_daily_actual_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_actual_4, '{}/results/actual/2017_daily_actual_vt_att.pkl'.format(baseUrl))
joblib.dump(daily_actual_5, '{}/results/actual/2018_daily_actual_vt_att.pkl'.format(baseUrl))
