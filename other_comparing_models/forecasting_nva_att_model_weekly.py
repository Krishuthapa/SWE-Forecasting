...
output_1 = '{}/outputs/forecasting_nva_weekly_att_tr_act_loss.txt'.format(baseUrl)
output_2 = '{}/outputs/forecasting_nva_weekly_att_ts_act_loss.txt'.format(baseUrl)
...
class CrossVarTransformer(nn.Module):
    def __init__(self, encoder_input_dim = 13 , batch_size = 7, sp_tp_dim = 6,  model_dim = 512, prompt_dim = 1536, enc_layers_count = 8, nhead = 16):
        ...
        # self.variable_embeddings = nn.Parameter(torch.zeros(1,encoder_input_dim, self.model_dim), requires_grad = True)
        # variable_embeddings = self.getVariableEmbeddingInitialized(self.variable_embeddings.shape[-1], np.arange(self.encoder_input_dim))
        # self.variable_embeddings.data.copy_(variable_embeddings.unsqueeze(0))
...
class SWETransformer(nn.Module):
    def __init__(self, model_dim = 1024, sp_tp_dim = 6, prompt_dim = 1536, n_output_heads = 1, enc_layers_count = 8, nhead = 16):
        ...
        self.historical_daily_att = CrossVarTransformer(batch_size = 30, model_dim = self.model_dim , prompt_dim = self.prompt_dim)
        self.historical_monthly_att = CrossVarTransformer(batch_size = 30, model_dim = self.model_dim, prompt_dim = self.prompt_dim)
...
def TrainModelSP(training_yr_indices, loc_prompts, location_distances, location_angularities, epoch_number):
    ...
    for index, input_index in enumerate(training_yr_indices):
        ...
        instance_monthly_input = getInputs(input_index, all_indices, window = 30, step_size = 7)
...
hasFile = os.path.isfile("{}/checkpoints/forecasting_nva_att_model_weekly.pt".format(baseUrl))
...
with open(output_1, 'w') as file1:
    with redirect_stdout(file1):
        ...
        if hasFile:
            checkpoint = torch.load("{}/checkpoints/forecasting_nva_att_model_weekly.pt".format(baseUrl))
        ...
        while strike_count <= 3 and epoch_number <= 10:
            ...
            if epoch_loss_sp < best_nse:
                ...
                }, "{}/checkpoints/forecasting_nva_att_model_weekly.pt".format(baseUrl))
...
def getFinalInputsForGP(training_yr_indices, train_prompts):
    ...
    with torch.no_grad():
        for index, input_index in enumerate(training_yr_indices):
            ...
            predicted_outputs.append(output.reshape(1,-1,4))
            actual_outputs.append(instance_output.reshape(1,-1,4))

    torch.save(input_learnt_representations, '{}/outputs/forecasting/forecasting_nva_weekly_att_inputs_for_gp_act.pt'.format(baseUrl))
    torch.save(predicted_outputs, '{}/outputs/forecasting/forecasting_nva_weekly_att_predicted_outputs_for_gp_act.pt'.format(baseUrl))
    torch.save(actual_outputs, '{}/outputs/forecasting/forecasting_nva_weekly_att_outputs_for_gp_act.pt'.format(baseUrl))
...
daily_forecasted_2014 = joblib.load('{}/results/predict/2014_daily_nva_predict.pkl'.format(baseUrl))
daily_forecasted_2015 = joblib.load('{}/results/predict/2015_daily_nva_predict.pkl'.format(baseUrl))
daily_forecasted_2016 = joblib.load('{}/results/predict/2016_daily_nva_predict.pkl'.format(baseUrl))
daily_forecasted_2017 = joblib.load('{}/results/predict/2017_daily_nva_predict.pkl'.format(baseUrl))
daily_forecasted_2018 = joblib.load('{}/results/predict/2018_daily_nva_predict.pkl'.format(baseUrl))
...
def getInputsWithForecastedSWE(current_yr_index, dataset_input_index, loc_indices, all_historical_available_swe, window = 1, step_size = 1):
    indices = sorted([dataset_input_index - (value * step_size) for value in range(0, window)])
    
    inputs = model_inputs[:,loc_indices,:][indices,:,:].reshape(window, len(loc_indices), -1)
    
    available_window_element_count = all_historical_available_swe.shape[0]

    if available_window_element_count <= window:
        selected_swe_inputs = all_historical_available_swe[-available_window_element_count:,:].reshape(available_window_element_count, len(loc_indices), 1)
    else:
        selected_swe_inputs = all_historical_available_swe[-window:,:].reshape(window, len(loc_indices), 1)
...
def getDailyPredictions(data_indices, daily_forecasted_values):
    actual_outputs = []
    ...
    predicted_indices = []
    collected_daily_forecasts = []

    daily_forecasted_values = daily_forecasted_values.transpose(1,0,2)
    ...
    for data_index in range(0,len(data_indices), forecasting_window * forecasting_step_size):
        ...
        instance_monthly_input = getInputs(data_indices[data_index], all_indices, window = 30, step_size = 7)
        ...
        output_loss = loss(output.reshape(-1,forecasting_window), instance_output.reshape(-1,forecasting_window))   

        predicted_indices = [data_index + 7 * value for value in range(1, forecasting_window+1)]
        ...
        if np.array(predicted_indices).max() >= len(data_indices):
            break

        predicted_indices_daily_forecast = daily_forecasted_values.reshape(-1,len(all_indices))[predicted_indices,:]
        collected_daily_forecasts.append(torch.tensor(predicted_indices_daily_forecast))
    ...
    print("Actual Daily Outputs", actual_daily_outputs.shape)
...
with open(output_2, 'w') as file1:
    with redirect_stdout(file1):
        print("Test Set 1")
        weekly_window_actual_1, weekly_window_predicted_1, test_1_final_representations  = TestModelSP(testing_yr_indices_1)
        weekly_actual_1, weekly_predicted_1 = getDailyPredictions(testing_yr_indices_1, daily_forecasted_2014)
        print("================================================")

        print("Test Set 2")
        weekly_window_actual_2, weekly_window_predicted_2, test_2_final_representations = TestModelSP(testing_yr_indices_2)
        weekly_actual_2, weekly_predicted_2 = getDailyPredictions(testing_yr_indices_2, daily_forecasted_2015)
        print("================================================")
        
        print("Test Set 3")
        weekly_window_actual_3, weekly_window_predicted_3, test_3_final_representations = TestModelSP(testing_yr_indices_3)
        weekly_actual_3, weekly_predicted_3 = getDailyPredictions(testing_yr_indices_3, daily_forecasted_2016)
        print("================================================")
        
        print("Test Set 4")
        weekly_window_actual_4, weekly_window_predicted_4, test_4_final_representations = TestModelSP(testing_yr_indices_4)
        weekly_actual_4, weekly_predicted_4 = getDailyPredictions(testing_yr_indices_4, daily_forecasted_2017)
        print("================================================")
        
        print("Test Set 5")
        weekly_window_actual_5, weekly_window_predicted_5, test_5_final_representations = TestModelSP(testing_yr_indices_5)
        weekly_actual_5, weekly_predicted_5 = getDailyPredictions(testing_yr_indices_5, daily_forecasted_2018)
        print("================================================")

sys.stdout = original_stdout


joblib.dump(weekly_window_predicted_1, '{}/results/predict/2014_weekly_nva_window_predict.pkl'.format(baseUrl))
joblib.dump(weekly_window_predicted_2, '{}/results/predict/2015_weekly_nva_window_predict.pkl'.format(baseUrl))
joblib.dump(weekly_window_predicted_3, '{}/results/predict/2016_weekly_nva_window_predict.pkl'.format(baseUrl))
joblib.dump(weekly_window_predicted_4, '{}/results/predict/2017_weekly_nva_window_predict.pkl'.format(baseUrl))
joblib.dump(weekly_window_predicted_5, '{}/results/predict/2018_weekly_nva_window_predict.pkl'.format(baseUrl))

joblib.dump(weekly_window_actual_1, '{}/results/actual/2014_weekly_nva_window_actual.pkl'.format(baseUrl))
joblib.dump(weekly_window_actual_2, '{}/results/actual/2015_weekly_nva_window_actual.pkl'.format(baseUrl))
joblib.dump(weekly_window_actual_3, '{}/results/actual/2016_weekly_nva_window_actual.pkl'.format(baseUrl))
joblib.dump(weekly_window_actual_4, '{}/results/actual/2017_weekly_nva_window_actual.pkl'.format(baseUrl))
joblib.dump(weekly_window_actual_5, '{}/results/actual/2018_weekly_nva_window_actual.pkl'.format(baseUrl))

joblib.dump(weekly_predicted_1, '{}/results/predict/2014_weekly_nva_predict.pkl'.format(baseUrl))
joblib.dump(weekly_predicted_2, '{}/results/predict/2015_weekly_nva_predict.pkl'.format(baseUrl))
joblib.dump(weekly_predicted_3, '{}/results/predict/2016_weekly_nva_predict.pkl'.format(baseUrl))
joblib.dump(weekly_predicted_4, '{}/results/predict/2017_weekly_nva_predict.pkl'.format(baseUrl))
joblib.dump(weekly_predicted_5, '{}/results/predict/2018_weekly_nva_predict.pkl'.format(baseUrl))

joblib.dump(weekly_actual_1, '{}/results/actual/2014_weekly_nva_actual.pkl'.format(baseUrl))
joblib.dump(weekly_actual_2, '{}/results/actual/2015_weekly_nva_actual.pkl'.format(baseUrl))
joblib.dump(weekly_actual_3, '{}/results/actual/2016_weekly_nva_actual.pkl'.format(baseUrl))
joblib.dump(weekly_actual_4, '{}/results/actual/2017_weekly_nva_actual.pkl'.format(baseUrl))
joblib.dump(weekly_actual_5, '{}/results/actual/2018_weekly_nva_actual.pkl'.format(baseUrl))
